<template>
  <div ref="target" class="a-select-search" :style="`${width ? 'width:' + width : ''}`" v-bind="$attrs">
    <div
      class="a-select-search__input"
      tabindex="0"
      aria-label="searchCity"
      aria-haspopup="listbox"
      :class="{ open: isOpen, checked: isChecked }"
      @click="onClickASelectInput"
      @keydown.enter.space.prevent="onClickASelectInput"
    >
      <div class="a-select-search__placeholder">
        {{ named }}
      </div>
    </div>
    <div class="a-select-search__list" :class="{ 'open': isOpen, 'visually-hidden': !(options && isOpen) }">
      <div v-if="withSearch" class="a-select-search__list-search">
        <AInput
          v-model="search"
          size="large"
          variant="noborder"
          ...
          class="a-select-search__search-input"
        />
        <div v-show="loading" class="a-select-search__search-loading"></div>
      </div>
      <ul
        ref="listOptions"
        role="listbox"
