<template>
  <PPage css-class="p-result">
        ...
        <ClientOnly>
          ...
          <div v-if="specs.length" class="specs-in-license">
            ...
            <div class="specs-in-license__message mb-8">
              ...
              <span
                >— При отсутствии аккредитации не предоставляется отсрочка от армии и выдается диплом установленного
                образца (на момент выдачи диплома специальность уже может быть аккредитована)</span
              >
            </div>
    ...
    <button v-show="false" ref="exportButton" @click.prevent="exportToPDF">Export to PDF</button>
  </PPage>
</template>
